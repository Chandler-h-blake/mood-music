...
import { Clock3, Database, GitCompareArrows, GripVertical, History, KeyRound, LibraryBig, ListMusic, LoaderCircle, Music2, Play, RefreshCw, Search, Settings2, Shuffle, Sparkles, Trash2, WandSparkles, Waves, WifiOff } from "lucide-react";
...
const PAGE_SIZE = 50;
type View = "library" | "discover" | "queue" | "history" | "settings";
...
export default function Home() {
  ...
  const title = { library: "我的喜欢", discover: "AI 选歌", queue: "临时队列", history: "队列历史", settings: "模型设置" }[view];
  const subtitle = { library: "QQ 音乐收藏的本地镜像", discover: "描述一种感觉，从全部喜欢中找到它", queue: "排序、删减、拖动或继续描述", history: "恢复每一次筛选、排序和人工调整快照", settings: "密钥只保存在本机 Windows 凭据存储" }[view];
  ...
  return <SidebarProvider style={{ "--sidebar-width": "15rem" } as React.CSSProperties} className="bg-[#07100e]">
    ...
    <SidebarInset className="min-w-0 bg-[#0b1412] text-[#eff8f3]">
      <header className="sticky top-0 z-10 flex min-h-20 items-center justify-between border-b border-white/[0.07] bg-[#0b1412]/90 px-4 backdrop-blur-xl sm:px-7"><div className="flex min-w-0 items-center gap-3"><SidebarTrigger className="text-white/60 hover:bg-white/[0.06] hover:text-white" /><div><h1 className="text-lg font-semibold sm:text-xl">{title}</h1><p className="mt-0.5 hidden text-sm text-white/42 sm:block">{subtitle}</p></div></div>{view === "library" ? <Button onClick={() => void syncLibrary()} disabled={syncing} className="h-10 rounded-xl bg-[#29e68b] text-[#04110c] hover:bg-[#50efa1]"><RefreshCw className={syncing ? "animate-spin" : ""} />{syncing ? "正在同步" : "同步 QQ 音乐"}</Button> : null}</header>
      <main className="mx-auto w-full max-w-[1440px] px-4 py-6 sm:px-7 sm:py-8">
        ...
        {view === "queue" ? <QueueView queue={queue} busy={queueBusy} goDiscover={() => setView("discover")} sortQueue={sortQueue} removeCandidate={removeCandidate} findSimilar={findSimilar} saveOrder={saveOrder} refineQueue={refineQueue} /> : null}
...
function QueueView({ queue, busy, goDiscover, sortQueue, removeCandidate, findSimilar, saveOrder, refineQueue }: QueueViewProps) {
