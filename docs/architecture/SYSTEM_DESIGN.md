...
MoodMusic 采用模块化单体、QQ 音乐数据适配器与可替换播放连接器组合。Next.js Web、FastAPI API、后台任务和 PostgreSQL 构成可独立运行的核心系统；数据适配器隔离 QQ 音乐只读内部接口，Windows PC 助手与备用 Chrome 扩展通过同一版本化本机协议接入，第三方细节不得渗透到搜索、画像或播放列表领域。当前实际播放开发以 PC 路线为主；UI Automation 与遗留 COM 均未通过，Windows 系统媒体会话只读状态已通过，歌曲定位仍待验证。
