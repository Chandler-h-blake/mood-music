...
上级目录中的 `QQ音乐AI选歌插件_产品与技术实施计划书.docx` 记录了最初的固定版本原生 Mod 方案。该文件只作为历史产品输入保留，不属于当前 Git 仓库，也不再控制当前系统架构。仍适用的业务规则已经迁移到当前 PRD；架构变化由 ADR 001、ADR 002 和 ADR 004 明确记录。
