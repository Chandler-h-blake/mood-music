...
> 当前状态（2026-09-13）：QQ 音乐曲库、DeepSeek 多角度画像、BGE-M3 语义选歌、三种排序、候选编辑和历史回放均已可用。两种播放器已共用版本化连接器协议；实机验证表明 QQ 音乐 22.41 的 UI Automation 控件树不可用，但客户端注册的 `QQMusicSvr 1.0` COM 服务已通过当前歌曲和队列三项只读查询。下一步是验证 PC 临时队列写入与播放；Chrome 实际播放暂停，外部推荐仍待开发。
...
| QQ 音乐 PC 连接器 | Windows 本地助手、客户端注册的 COM 自动化、系统媒体能力 | 在不修改或注入客户端的前提下控制桌面播放器；按版本逐项验证能力 |
