...
本仓库采用“独立 Web 应用 + 本机 QQ 音乐数据适配器 + 可替换播放连接器”的产品形态。MoodMusic 后端使用用户手动粘贴并由 Windows 安全存储保护的 QQ 音乐 Cookie，只读同步当前账号的“我喜欢”；Chrome 扩展将作为第一条稳定播放路径，Windows PC 连接器在可行性验证通过后接入同一队列协议。项目不修改或注入 QQ 音乐安装文件，不下载或转发受保护音频，也不绕过会员、地区、版权或 DRM 限制。

> 当前状态（2026-09-13）：QQ 音乐曲库纵向切片已通过真实账号验证。系统现可安全保存 DeepSeek API Key，增量建立歌曲多角度画像与 BGE-M3 1024 维语义向量，统一常见语言、人声和曲风标签，理解用户输入的感觉并执行明确的人声、语言、纯音乐与情绪边界，全量评估当前“喜欢”歌曲，并支持三种排序、候选删除与拖动、找相似、追加要求和队列历史回放。Chrome 网页播放连接器、QQ 音乐 PC 连接器和外部推荐仍待开发；PC 连接器须先通过可行性门槛。MoodMusic 是暂定产品名，可以通过架构决策记录统一更名。
...
7. 用户选择并配对可用播放连接器；Chrome 扩展优先交付，PC 助手在可行性验证通过后接入相同队列协议。
...
| QQ 音乐网页连接器 | TypeScript、Chrome Extension Manifest V3 | 接收候选队列、定位歌曲、驱动网页播放器并回传状态 |
| QQ 音乐 PC 连接器 | Windows 本地助手、UI Automation/公开系统能力 | 在不修改或注入客户端的前提下控制桌面播放器；先通过可行性验证 |
...
- QQ 音乐 PC 连接器完成可行性验证，并在验证通过后接入相同的队列与能力协商协议。
