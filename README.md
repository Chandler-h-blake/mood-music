...
> 当前状态（2026-09-13）：QQ 音乐曲库、DeepSeek 多角度画像、BGE-M3 语义选歌、三种排序、候选编辑和历史回放均已可用。两种播放器已共用版本化连接器协议；实机验证表明 QQ 音乐 22.41 的 UI Automation 控件树不可用，注册的 `QQMusicSvr 1.0` COM 服务也与现代客户端播放脱节。下一步验证 PC 的 Windows 系统媒体会话和安全歌曲定位；Chrome 实际播放暂停，外部推荐仍待开发。
